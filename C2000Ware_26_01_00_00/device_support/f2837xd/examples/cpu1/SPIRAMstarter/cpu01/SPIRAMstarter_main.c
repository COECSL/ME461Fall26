//#############################################################################
// FILE:   SPIRAMstarter_main.c
//
// TITLE:  SPIRAM Starter
//#############################################################################
...
#include "driverlib.h"
#include "device.h"
#warn "NOTE, buffer.h needs to be modified changing BUF_SIZE  (1024) if it is not already for the Matlab read funciton"
 ...
 * NUM_PACKET is the total number of packets to read
 * ############################################################################
 */
...
void main(void)
{
    ...
    {
        if (UARTPrint == 1 ) {
			// Can't print to SerialA if using to send SPIRAM data to Matlab
            UARTPrint = 0;
        }
    }
}
